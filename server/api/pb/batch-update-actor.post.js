...
export default defineEventHandler(async (event) => {
  ...
  const { recordIds, newActor } = await readBody(event);
  ...
  if (!newActor) {
    throw createError({ statusCode: 400, message: 'No actor value provided' });
  }
  ...
  try {
    ...
    console.error('Error in batch update actor endpoint:', error.message);
    throw createError({ statusCode: 500, message: 'Failed to update actor' });
  }
});
