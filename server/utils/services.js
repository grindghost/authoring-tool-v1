  ...
  const processElement = (el, indentLevel = 0) => {
    ...
    if (tag === 'h1' || tag === 'h2' || tag === 'h3' || tag === 'p') {
      ...
      $(el).children().each((j, li) => {
        ...
        // Check the `data-list` attribute on the `LI` element
        const listType = $(li).attr('data-list');

        const bullet = listType === 'bullet'  // If the list item has "bullet", use the bullet point
          ? '•'
          : listType === 'ordered'  // If the list item has "ordered", use the ordered list format
            ? getOrderedListBullet(level, listNumbering[level] - 1)
            : ''; // Default case (if no attribute is present, this shouldn't happen)
...
export const convertQuillHtmlToText_v2 = (quillHtml) => {
  const $ = cheerio.load(quillHtml);
  let result = '';
